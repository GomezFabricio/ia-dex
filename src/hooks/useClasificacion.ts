...
// ---------------------------------------------------------------------------
// useClasificacion — fetches a single ClasificacionSI by slug with active-flag stale guard
// Extends the canonical useTemas reducer template with two additions:
...
type Action =
  | { type: 'pending' }
  | { type: 'refetch' }
  | { type: 'success'; payload: ClasificacionSI | null }
...
export function useClasificacion(slug: string): {
  data: ClasificacionSI | null
  loading: boolean
  error: Error | null
  refetch: () => void
