...
// ---------------------------------------------------------------------------
// useSoftwarePorClasificacion — software linked to a clasificacion_si.
// Skip variant: clasificacionId undefined → success([]) (dependency not ready).
...
export function useSoftwarePorClasificacion(clasificacionId: string | undefined): {
  data: Software[]
  loading: boolean
  error: Error | null
  refetch: () => void
  ...
  useEffect(() => {
    ...
    softwareService
      .listarPorClasificacion(clasificacionId)
      .then((items) => {
